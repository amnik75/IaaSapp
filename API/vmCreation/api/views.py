    ...
    def post(self, request, format= None):
        ...
        if serializer.is_valid():
            ...
            if server.status == "Error":
                ...
                server.host = None
                server.save()
                result = subprocess.run(['ssh',ip,'sudo','./vmEvacuate.sh',request.data['name'],str(int(request.data['ram']) * 1024),request.data['cpu'],server.mac,server.ip],stdout=PIPE, stderr=PIPE)
                s = Server.objects.filter(name = request.data['name']).first()
                h = Host.objects.filter(id = host).first()
                s.status = "Active"
                s.host = h
